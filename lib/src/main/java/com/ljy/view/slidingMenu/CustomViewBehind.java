...
public class CustomViewBehind extends ViewGroup {
	...
	private int mTouchMode = LjySlidingMenu.TOUCHMODE_MARGIN;
	...
	private LjySlidingMenu.CanvasTransformer mTransformer;
	...
	public void setCanvasTransformer(LjySlidingMenu.CanvasTransformer t) {
		mTransformer = t;
	}
	...
	public void setMode(int mode) {
		if (mode == LjySlidingMenu.LEFT || mode == LjySlidingMenu.RIGHT) {
			if (mContent != null)
	...
	public int getMenuPage(int page) {
		...
		if (mMode == LjySlidingMenu.LEFT && page > 1) {
			return 0;
		} else if (mMode == LjySlidingMenu.RIGHT && page < 1) {
			return 2;
		} else {
			return page;
		}
	...
	public int getAbsLeftBound(View content) {
		if (mMode == LjySlidingMenu.LEFT || mMode == LjySlidingMenu.LEFT_RIGHT) {
			return content.getLeft() - getBehindWidth();
		} else if (mMode == LjySlidingMenu.RIGHT) {
			return content.getLeft();
		}
	...
	public int getAbsRightBound(View content) {
		if (mMode == LjySlidingMenu.LEFT) {
			return content.getLeft();
		} else if (mMode == LjySlidingMenu.RIGHT || mMode == LjySlidingMenu.LEFT_RIGHT) {
			return content.getLeft() + getBehindWidth();
		}
		return 0;
	}
	...
	public boolean menuOpenTouchAllowed(View content, int currPage, float x) {
		switch (mTouchMode) {
		case LjySlidingMenu.TOUCHMODE_FULLSCREEN:
			return true;
		case LjySlidingMenu.TOUCHMODE_MARGIN:
	...
	public boolean menuTouchInQuickReturn(View content, int currPage, float x) {
		if (mMode == LjySlidingMenu.LEFT || (mMode == LjySlidingMenu.LEFT_RIGHT && currPage == 0)) {
			return x >= content.getLeft();
		} else if (mMode == LjySlidingMenu.RIGHT || (mMode == LjySlidingMenu.LEFT_RIGHT && currPage == 2)) {
