...
public class ViewDragHelper {
    ...
    public void setSensitivity(Context context, float sensitivity) {
        float s = Math.max(0.0F, Math.min(1.0F, sensitivity));
        ViewConfiguration viewConfiguration = ViewConfiguration.get(context);
        this.mTouchSlop = (int) ((float) viewConfiguration.getScaledTouchSlop() * (1.0F / s));
    }
    ...
    private void saveInitialMotion(float x, float y, int pointerId) {
        ...
        this.mInitialEdgeTouched[pointerId] = this.getEdgeTouched((int) x, (int) y);
        this.mPointersDown |= 1 << pointerId;
    }
        ...
        switch (action) {
            ...
            case 2:
                ...
                for (int i = 0; i < pointerId; ++i) {
                    ...
                    View toCapture2 = this.findTopChildUnder((int) x2, (int) y2);
                    if (toCapture2 != null && this.checkTouchSlop(toCapture2, dx, dy) && this.tryCaptureViewForDrag(toCapture2, pointerId)) {
                        break;
                    }
                }
            ...
            case 5:
                ...
                this.saveInitialMotion(x2, y2, pointerId);
                if (this.mDragState == 0) {
                    int edgesTouched2 = this.mInitialEdgeTouched[pointerId];
                    if ((edgesTouched2 & this.mTrackingEdges) != 0) {
                        this.mCallback.onEdgeTouched(edgesTouched2 & this.mTrackingEdges, pointerId);
                    }
                } else if (this.mDragState == 2) {
                    toCapture = this.findTopChildUnder((int) x2, (int) y2);
    ...
    public void processTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case 0:
                ...
                this.saveInitialMotion(x, y, pointerId);
                this.tryCaptureViewForDrag(toCapture, pointerId);
                id = this.mInitialEdgeTouched[pointerId];
            ...
            case 2:
                if (this.mDragState == 1) {
                    ...
                    x = MotionEventCompat.getX(ev, pointerId);
                    y = MotionEventCompat.getY(ev, pointerId);
                    ...
                    for (newActivePointer = 0; newActivePointer < pointerId; ++newActivePointer) {
                        pointerId = MotionEventCompat.getPointerId(ev, newActivePointer);
                        float x2 = MotionEventCompat.getX(ev, newActivePointer);
                        float y2 = MotionEventCompat.getY(ev, newActivePointer);
                        x2 = x2 - this.mInitialMotionX[pointerId];
                        y2 = y2 - this.mInitialMotionY[pointerId];
                        this.reportNewEdgeDrags(x2, y2, pointerId);
                        if (this.mDragState == 1) {
                            break;
                        }

                        View toCapture2 = this.findTopChildUnder((int) x2, (int) y2);
                        if (this.checkTouchSlop(toCapture2, x2, y2) && this.tryCaptureViewForDrag(toCapture2, pointerId)) {
                            break;
                        }
                    }
    ...
    private boolean checkNewEdgeDrag(float delta, float odelta, int pointerId, int edge) {
        ...
        if ((this.mInitialEdgeTouched[pointerId] & edge) == edge && (this.mTrackingEdges & edge) != 0 && (this.mEdgeDragsLocked[pointerId] & edge) != edge && (this.mEdgeDragsInProgress[pointerId] & edge) != edge && (absDelta > (float) this.mTouchSlop || absODelta > (float) this.mTouchSlop)) {
    ...
    public boolean isEdgeTouched(int edges, int pointerId) {
        return this.isPointerDown(pointerId) && (this.mInitialEdgeTouched[pointerId] & edges) != 0;
    }
