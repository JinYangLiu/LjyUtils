...
/**
 * Created by Mr.LJY on 2018/1/10.
 * <p>
 * 标准播放器，继承之后实现一些ui显示效果，如显示／隐藏ui，播放按键等
 */

public class LjyVideoPlayer extends GSYVideoPlayer {
     ...
     */
    public LjyVideoPlayer(Context context, Boolean fullFlag) {
        super(context, fullFlag);
    }

    public LjyVideoPlayer(Context context) {
        super(context);
    }

    public LjyVideoPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void startPlayLogic() {
        if (mStandardVideoAllCallBack != null) {
            ...
            mStandardVideoAllCallBack.onClickStartThumb(mOriginUrl, mTitle, LjyVideoPlayer.this);
        }
        prepareVideo();
    ...
    protected void cloneParams(GSYBaseVideoPlayer from, GSYBaseVideoPlayer to) {
        ...
        LjyVideoPlayer sf = (LjyVideoPlayer) from;
        LjyVideoPlayer st = (LjyVideoPlayer) to;
    ...
    public GSYBaseVideoPlayer startWindowFullscreen(Context context, boolean actionBar, boolean statusBar) {
        ...
        if (gsyBaseVideoPlayer != null) {
            LjyVideoPlayer videoPlayer = (LjyVideoPlayer) gsyBaseVideoPlayer;
            videoPlayer.setStandardVideoAllCallBack(mStandardVideoAllCallBack);
     ...
     * 全屏的UI逻辑
     */
    private void initFullUI(LjyVideoPlayer ljyVideoPlayer) {

        if (mBottomProgressDrawable != null) {
            ljyVideoPlayer.setBottomProgressBarDrawable(mBottomProgressDrawable);
        }

        if (mBottomShowProgressDrawable != null && mBottomShowProgressThumbDrawable != null) {
            ljyVideoPlayer.setBottomShowProgressBarDrawable(mBottomShowProgressDrawable,
                    mBottomShowProgressThumbDrawable);
        }

        if (mVolumeProgressDrawable != null) {
            ljyVideoPlayer.setDialogVolumeProgressBar(mVolumeProgressDrawable);
        }

        if (mDialogProgressBarDrawable != null) {
            ljyVideoPlayer.setDialogProgressBar(mDialogProgressBarDrawable);
        }

        if (mDialogProgressHighLightColor >= 0 && mDialogProgressNormalColor >= 0) {
            ljyVideoPlayer.setDialogProgressColor(mDialogProgressHighLightColor, mDialogProgressNormalColor);
        }
    }
